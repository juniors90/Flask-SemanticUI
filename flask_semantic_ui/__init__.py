...
SEMANTIC_UI_VERSION = re.sub(r'^(\d+\.\d+\.\d+).*', r'\1', __version__)
JQUERY_VERSION = '3.6.0'
HTML5SHIV_VERSION = '3.7.3'
RESPONDJS_VERSION = '2.4.1'
...
def semantic_ui_find_resource(filename, cdn, use_minified=None, local=True, jquery=None):
...
class SemanticUI(object):
    ...
    def init_app(self, app):
        ...
        app.jinja_env.globals['semantic_ui_is_hidden_field'] =\
            is_hidden_field_filter
        app.jinja_env.globals['semantic_ui_find_resource'] =\
        ...
        semantic_ui = lwrap(
            WebCDN(f'https://cdnjs.cloudflare.com/ajax/libs/semantic-ui/{SEMANTIC_UI_VERSION}/'), local)

        jquery = lwrap(
            WebCDN(f'https://code.jquery.com/jquery-{JQUERY_VERSION}'))

        html5shiv = lwrap(
            WebCDN(f'//cdnjs.cloudflare.com/ajax/libs/html5shiv/{HTML5SHIV_VERSION}/'))

        respondjs = lwrap(
            WebCDN(f'https://cdnjs.cloudflare.com/ajax/libs/semantic-ui/{RESPONDJS_VERSION}'), local)
